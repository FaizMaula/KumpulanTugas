...
func MenuTampilData() {
	...
	for !menu {
		...
		fmt.Println("7. Rekapitulasi Kegiatan Per Tahun")
		fmt.Println("8. Kembali ke Menu")
		fmt.Println("===============================================")
...
func fakultasSort(T *arrPenelitian, n int) {
	...
	for pass < n {
		...
		i := pass
		for i < n {
			if T[idx].fakultas > T[i].fakultas {
				idx = i
			}
			i++
		}
...
func tahunSort(T *arrPenelitian, n int) {
	pass := 1
	for pass < n {
		idx := pass - 1
		i := pass
...
func totalSort(T *arrPenelitian, n int) {
	fakultasSort(T, n)
	tahunSort(T, n)
	namaSort(T, n)
}
...
func cariArray(T arrPenelitian, B *arrPenelitian, n int, x string, y int) {
	var hasil bool = false
	j := 0
	if y == 3 {
...
func Rekapitulasi(T arrPenelitian, n int) {
	type rekap struct {
		tahun   int
		jumlah1 int
	}
	type arrRekap [10]rekap
	x := arrRekap{
		{2017, 0},
		{2018, 0},
		{2019, 0},
	...
	for i := 0; i < n; i++ {
		if T[i].tahun == 2017 {
			x[0].jumlah1++
		} else if T[i].tahun == 2018 {
			x[1].jumlah1++
		} else if T[i].tahun == 2019 {
			x[2].jumlah1++
		} else if T[i].tahun == 2020 {
			x[3].jumlah1++
		} else if T[i].tahun == 2021 {
			x[4].jumlah1++
...
func mengeditData( T *arrPenelitian, n int) {
	...
	for {
		...
		if idx != -1 {
			...
			fmt.Print("Pilihan: ");fmt.Scan(&opsi);fmt.Print("\n")
			for opsi ==  "1" {
